...
export class Inventory {
  constructor(game) { 
    ...
    setTimeout(() => {
      // Only add items that actually exist in our ITEMS object
      if (ITEMS.bronze_bar) this.addItem(ITEMS.bronze_bar);
      if (ITEMS.wood) this.addItem(ITEMS.wood); // Using wood instead of rotten_log
      if (ITEMS.cloth_chest) this.addItem(ITEMS.cloth_chest);
      if (ITEMS.pine_log) this.addItem(ITEMS.pine_log);
      if (ITEMS.fir_log) this.addItem(ITEMS.fir_log);
      if (ITEMS.maple_log) this.addItem(ITEMS.maple_log);
      if (ITEMS.birch_log) this.addItem(ITEMS.birch_log);
      if (ITEMS.copper) this.addItem(ITEMS.copper); // Using copper instead of copper_ore
      if (ITEMS.copper_bar) this.addItem(ITEMS.copper_bar);
    }, 100);
  }
  ...
  showTooltip(index, x, y, specificItem = null) {
    ...
    // Generate icon HTML based on item type (SVG or image)
    let iconHTML;
    if (item.useImageTag) {
      // Use direct image tag for items with useImageTag flag
      iconHTML = `<img src="${item.icon}" alt="${item.name}" class="tooltip-icon" width="24" height="24">`;
    } else if (item.icon.startsWith('<svg')) {
      // For SVG content
      iconHTML = item.icon;
    } else {
      // Fallback to direct image path
      iconHTML = `<img src="${item.icon}" alt="${item.name}" class="tooltip-icon">`;
    }
    
    this.tooltip.innerHTML = `
      <div class="tooltip-header">
        <div class="tooltip-icon-container">${iconHTML}</div>
        <h4>${item.name}</h4>
  ...
  addItem(item) {
    ...
    if (!item) {
      console.warn('Attempted to add undefined or null item to inventory');
      return false;
    }
    ...
    for (let i = 0; i < totalSlots; i++) {
      ...
      if (item) {
        ...
        if (this.currentCategory !== 'all' && item.type !== this.currentCategory) {
          ...
          // Insert the item's icon based on type (SVG or image)
          if (item.useImageTag) {
            // For items with useImageTag flag
            const iconContainer = document.createElement('div');
            iconContainer.className = 'iv-item-icon-container';
            iconContainer.innerHTML = `<img src="${item.icon}" alt="${item.name}" class="iv-item-icon" width="24" height="24">`;
